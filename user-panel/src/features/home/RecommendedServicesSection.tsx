...
// Main RecommendedCoursesSection Component
function RecommendedCoursesSection({ router }: { router: AppRouterInstance }) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          ...
          <p className="text-base sm:text-lg text-slate-600 max-w-2xl mx-auto px-4 sm:px-0">
            Handpicked courses designed for Muslim professionals
          </p>
        </div>

        {/* Featured Course - Hero Layout */}
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-2 gap-0">
              ...
              <div className="p-6 sm:p-8 lg:p-12 text-white order-2 lg:order-1">
                <div className="mb-3 sm:mb-4">
                  ...
                  {featuredCourse.discountEnabled && featuredCourse.discount > 0 && (
                    <span className="bg-red-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold ml-2">
                      {featuredCourse.discount}% OFF
                    </span>
                  )}
                </div>
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-2 sm:mb-3">{featuredCourse.title}</h3>
                <p className="text-emerald-100 mb-1 sm:mb-2 font-medium text-sm sm:text-base">{featuredCourse.tagline}</p>
                <p className="text-white/80 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base">{featuredCourse.description}</p>
                
                {/* Learning Points */}
                <div className="mb-4 sm:mb-6">
                  <h4 className="text-white font-semibold mb-2 sm:mb-3 text-sm sm:text-base">What you'll learn:</h4>
                  <div className="grid grid-cols-1 gap-1.5 sm:gap-2">
                    {featuredCourse.learningPoints.slice(0, 3).map((point, index) => (
                ...
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8 text-xs sm:text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{featuredCourse.averageReviewScore}</span>
                    <span className="text-white/60">({featuredCourse.totalReviewCount})</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <PlayIcon className="w-3 h-3 sm:w-4 sm:h-4" />
                    <span className="hidden sm:inline">{featuredCourse.enrollmentCount} students</span>
                    <span className="sm:hidden">{featuredCourse.enrollmentCount}</span>
                  </div>
                  <div className="text-white/80">
                    {featuredCourse.duration} min
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <span className="text-2xl sm:text-3xl font-bold">
                      ${calculateDiscountedPrice(featuredCourse.price, featuredCourse.discount, featuredCourse.discountEnabled)}
                    </span>
                    {featuredCourse.discountEnabled && featuredCourse.discount > 0 && (
                      <span className="text-white/60 line-through ml-2 text-lg sm:text-xl">${featuredCourse.price}</span>
                    )}
        ...
        {/* Other Courses - Responsive List */}
        <div className="space-y-3 sm:space-y-4">
          <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-4 sm:mb-6 px-4 sm:px-0">More Recommended Courses</h3>
              ...
              <div className="flex flex-col sm:grid sm:grid-cols-4 gap-4 sm:gap-6 sm:items-center">
                ...
                <div className="sm:col-span-2">
                  <div className="mb-2">
                    ...
                    {course.discountEnabled && course.discount > 0 && (
                  ...
                  <p className="text-emerald-600 text-sm font-medium mb-2">{course.tagline}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-slate-500">
                    <div className="flex items-center gap-1">
                      ...
                      {course.averageReviewScore} ({course.totalReviewCount})
                    </div>
                    <span className="hidden sm:inline">{course.enrollmentCount} students</span>
                    <span className="sm:hidden">{course.enrollmentCount}</span>
                ...
                <div className="sm:col-span-1 flex sm:flex-col justify-between sm:text-right items-end sm:items-end gap-4">
                  <div className="sm:mb-4">
                    <span className="text-xl sm:text-2xl font-bold text-slate-900">
                      ${calculateDiscountedPrice(course.price, course.discount, course.discountEnabled)}
                    </span>
                    {course.discountEnabled && course.discount > 0 && (
                      <div className="text-sm text-slate-400 line-through">${course.price}</div>
                    )}
                  </div>
                  <button 
                    onClick={(e) => {
