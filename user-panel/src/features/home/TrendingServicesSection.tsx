...
const ServiceCard = ({ service, idx, router }: { service: Service; idx: number; router: ReturnType<typeof useRouter> }) => {
  ...
  return (
    <div 
      ...
      className="relative bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer active:scale-95 sm:active:scale-100"
    >
      <div className="absolute top-2 sm:top-4 left-2 sm:left-4 z-10 flex items-center gap-1 bg-gradient-to-r from-emerald-500 to-green-600 text-white px-2 py-1 rounded-full shadow-md">
        <TrendingUp className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
        <span className="text-white font-bold text-xs sm:text-sm">#{idx + 1} Trending</span>
      </div>
      ...
      <div className="relative h-40 sm:h-48">
        ...
        <div className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 right-2 sm:right-4 text-white">
          <div className="flex items-center gap-2 sm:gap-3">
            <span className="flex items-center gap-1 bg-white/20 backdrop-blur-md px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg">
              <Star className="w-3 h-3 sm:w-4 sm:h-4 fill-yellow-400 text-yellow-400" />
              <span className="font-bold text-xs sm:text-sm">{service.averageReviewScore || '0.0'}</span>
            </span>
            <span className="bg-white/20 backdrop-blur-md px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg text-xs sm:text-sm">
      ...
      <div className="p-3 sm:p-4">
        <h3 className="font-bold text-slate-900 text-base sm:text-lg mb-1.5 sm:mb-2 line-clamp-2">{service.title}</h3>
        <p className="text-slate-600 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-2">{service.tagline}</p>
        <Button variant="primary" className="w-full text-sm sm:text-base py-2">Join the Trend</Button>
      </div>
    </div>
  )
}
    ...
    <section className="py-10 sm:py-16 bg-gradient-to-br from-emerald-50 via-teal-50 to-green-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center max-w-md mx-auto">
          <ServiceCard key={trendingServices[0].id} service={trendingServices[0]} idx={0} router={router} />
        </div>
      </div>
    </section>
  );
    ...
    <section className="py-8 sm:py-12 bg-gradient-to-br from-emerald-50 via-teal-50 to-green-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-4xl mx-auto">
          <ServiceCard key={topServices[0].id} service={topServices[0]} idx={0} router={router} />
          <ServiceCard key={topServices[1].id} service={topServices[1]} idx={1} router={router} />
        </div>
      </div>
    </section>
  )
