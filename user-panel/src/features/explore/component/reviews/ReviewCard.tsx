...
<div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm hover:bg-gray-50 transition-colors duration-200">
  <div className="flex flex-wrap items-center gap-4">
    ...
    <Avatar.Root className="h-12 w-12 shrink-0 rounded-full bg-violet-200 grid place-items-center text-white font-semibold">
      {review.userImageUrl ? (
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = "/images/coverImage.png";
          }}
        />
      ) : (
    ...
    <div className="min-w-0 flex-1">
      <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2">
        ...
        {userData?.country && (
          <span className="text-sm text-gray-500">{userData.country}</span>
        )}
      </div>
      ...
      <div className="mt-1 flex items-center flex-wrap gap-1">
        ...
        <span className="text-sm font-semibold text-gray-600">{review.stars}</span>
  ...
  {/* Review Text */}
  <p className="mt-4 text-[15px] leading-6 text-gray-800 break-words">
