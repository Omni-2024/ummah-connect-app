...
const FilterBar = ({ resetPage }: { resetPage: () => void }) => {
  ...
  const cmeUp = useSelector(selectCMEUp) ?? MAX_CME_POINTS;
  const cmeDown = useSelector(selectCMEDown) ?? 0;
  ...
  const handleProfessionChange = (professionId: string) => {
    ...
    if (profession === professionId) {
      dispatch(exploreActions.resetFilters()); // Uncheck the radio button
    } else {
      dispatch(
        exploreActions.setProfession({
          profession: professionId,
          professionName:
            categories?.find((cat) => cat.id === professionId)?.name || "",
        }),
      );
    }
  };
