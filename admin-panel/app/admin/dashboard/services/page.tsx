...
import { useCurrentUser } from "@/hooks/useUserInfo"; // ✅ use same hook as PersonalInfo
...
export default function AdminGigsPage() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<ServiceCategoryFilterData | []>([]);
  ...
  // ✅ Fetch services only if user is loaded
        const {
        data: services,
        isLoading,
        refetch: refetchServices,
        } = useServices(userId ? serviceParams : ({} as GetAllServiceParams));
  ...
  return (
    <div>
      ...
      <div className="flex justify-between py-8 px-4">
        ...
        />
        <form className="flex gap-3 px-2" onSubmit={(e) => e.preventDefault()}>
          <Input
